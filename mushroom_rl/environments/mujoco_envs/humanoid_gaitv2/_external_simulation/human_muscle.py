...
class TIA(MuscleTendonComplex):
    ...
    def __init__(self, angAnk, timestep):
        ...
        super().__init__(nameMuscle=nameMuscle, frcmax=frcmax, vmax=vmax, lslack=lslack, lopt=lopt,
                         ...
                         offsetCorr=offsetCorr, timestep=timestep, angJoi=angJoi)
...
class HFL(MuscleTendonComplex):
    ...
    def __init__(self, angHip, timestep):
        ...
        super().__init__(nameMuscle=nameMuscle, frcmax=frcmax, vmax=vmax, lslack=lslack, lopt=lopt,
                         lce=lce, r=r, phiref=phiref, phimaxref=phimaxref, rho=rho, dirAng=dirAng, phiScale=phiScale,
                         offsetCorr=offsetCorr, timestep=timestep, angJoi=angJoi, act=act)
...
class HROE(MuscleTendonComplex):
    """
...
class HROI(MuscleTendonComplex):
    """
    ...
    def __init__(self, angHipFront, timestep):
        ...
        rHRO = 0.06  # [m]   constant lever contribution
        phirefHRO = 10 * np.pi / 180  # [rad] reference angle at which MTU length equals
        rhoHRO = 0.7  # sum of lopt and lslack

        r = np.array((rHRO,))
        phiref = np.array((phirefHRO,))
        phimaxref = np.array((0.0,))
        rho = np.array((rhoHRO,))
        dirAng = np.array((-1.0,))
        offsetCorr = np.array((0,))
